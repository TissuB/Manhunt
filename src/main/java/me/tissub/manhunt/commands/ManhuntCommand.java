package me.tissub.manhunt.commands;
...
public class ManhuntCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args[0].equals("help")) {
            ...
            p.sendMessage(ChatColor.YELLOW + "/manhunt add <role> <player> <player> ... " + ChatColor.AQUA
                    + "- adds players to a manhunt game with roles");
            p.sendMessage(ChatColor.YELLOW + "/manhunt remove <player> <player> ..." + ChatColor.AQUA
                    + "- removes players from a manhunt game");
            ...
            p.sendMessage(ChatColor.YELLOW + "/manhunt list " + ChatColor.AQUA
                    + "- shows a list of players in a manhunt game with their roles");
        ...
        if (args[0].equals("add")) {
            ...
            if (args[1].equals("speedrunner")) {
                for (int i = 2; i < args.length; i++) {
                    ...
                    if (target == null || speedrunners.contains(target.getName())) {
                        continue;
                    }
                ...
                if (count > 0) {
                    p.sendMessage(ChatColor.AQUA + "Successfully added " + count + " new speedrunner"
                            + (count == 1 ? "" : "s") + " to the game!");
                } else {
                    p.sendMessage(
                            ChatColor.RED + "Could not add " + (args.length == 3 ? "this player!" : "these players!"));
                }
                return true;
            }
            if (args[1].equals("hunter")) {
                for (int i = 2; i < args.length; i++) {
                    target = Bukkit.getPlayerExact(args[i]);
                    if (target == null || hunters.contains(target.getName())) {
                        continue;
                    }
                ...
                if (count > 0) {
                    p.sendMessage(ChatColor.AQUA + "Successfully added " + count + " new hunter"
                            + (count == 1 ? "" : "s") + " to the game!");
                } else {
                    p.sendMessage(
                            ChatColor.RED + "Could not add " + (args.length == 3 ? "this player!" : "these players!"));
        ...
        if (args[0].equals("remove")) {
            ...
            for (int i = 1; i < args.length; i++) {
                target = Bukkit.getPlayerExact(args[i]);
                if (target == null
                        || (inGame && (speedrunners.contains(target.getName()) && speedrunners.size() == 1
                                || hunters.contains(target.getName()) &&
                                        hunters.size() == 1))
                        || (!speedrunners.contains(target.getName()) && !hunters.contains(target.getName()))) {
                    continue;
                }
            ...
            if (count > 0) {
                ...
                p.sendMessage(
                        ChatColor.RED + "Could not remove " + (args.length == 2 ? "this player!" : "these players!"));
            }
            return true;
        }
        if (args[0].equals("start")) {
            ...
            starting = new BukkitRunnable() {
                @Override
                public void run() {
                    playersMessage(ChatColor.BLUE + String.valueOf(seconds) + " second" + (seconds == 1 ? "" : "s")
                            + " remaining!");
                        ...
                        if (compassMode.get(i).equals("0")) {
                            ...
                            for (String s : speedrunners) {
                                ...
                                if (hunter.getWorld().getEnvironment()
                                        .equals(speedrunner.getWorld().getEnvironment())) {
                            ...
                            if (target == null) {
                                ...
                                meta.setDisplayName(
                                        ChatColor.GOLD + "Tracking: " + ChatColor.GREEN + "nearest speedrunner");
                                hunter.getInventory().getItem(compassSlot(hunter)).setItemMeta(meta);
                            }
                        } else {
                            CompassMeta meta = (CompassMeta) compass.getItemMeta();
                            ...
                            if (target == null) {
                                ...
                                if (!target.getWorld().getEnvironment().equals(hunter.getWorld().getEnvironment())) {
                                    meta.setDisplayName(
                                            ChatColor.RED + target.getName() + " is not in this dimension!");
                                } else if (target.isDead()) {
                                    meta.setDisplayName(ChatColor.RED + target.getName() + " is dead!");
                                } else {
                                    meta.setDisplayName(
                                            ChatColor.GOLD + "Tracking: " + ChatColor.GREEN + whichSpeedrunner.get(i));
                                }
                                if (main.getConfig().getBoolean("trackPortals") || target.getWorld().getEnvironment()
                                        .equals(hunter.getWorld().getEnvironment())) {
        ...
        if (args[0].equals("pause")) {
            ...
            playersMessage(ChatColor.AQUA + p.getName() + " wants to pause the game! (" + pausePlayers.size() + "/"
                    + (hunters.size() + speedrunners.size()) + ")");
            if (pausePlayers.size() == hunters.size() + speedrunners.size()) {
                pausing.cancel();
        ...
        if (args[0].equals("unpause")) {
            ...
            playersMessage(ChatColor.AQUA + p.getName() + " wants to unpause the game! (" + unpausePlayers.size() + "/"
                    + (hunters.size() + speedrunners.size()) + ")");
            if (unpausePlayers.size() == hunters.size() + speedrunners.size()) {
                unpausing.cancel();
                pausePlayers.clear();
